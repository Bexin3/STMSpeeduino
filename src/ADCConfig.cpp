...
int FastAnalogRead(int Pin) { //Fix the double remap
    
    for (int i = 0; i < 3; i++) {
        if (ADCInitialized[i] && ADCPinRemap(ADCList[i], Pin) != -1) {
            ADCInjectedRead(ADCList[i], Pin);
            return(-1);
        };
        };
    if (ADCPinRemap(ADC1, Pin) != -1) {ADCInjectedRead(ADC1, Pin);
    } else {ADCInjectedRead(ADC3, Pin);};
...
int ADCInjectedRead(ADC_TypeDef * ADC, int ADCChannel) {
   if (!ADCInitialized[GetADCNumber(ADC)]) { //Fix
    ...
    while (!READ_REG(ADC->ISR & ADC_ISR_JEOC) && (micros() < (0xFFF + InitMicros))) {}; //Wait for a new value if the latest one was already read
